...
import '../viewmodels/internet_form_viewmodel.dart';
...
class InternetFormPage extends ConsumerStatefulWidget {
  final String? initialSku;
  final String? initialName;

  const InternetFormPage({
    super.key,
    this.initialSku,
    this.initialName,
  });
  ...
  @override
  ConsumerState<InternetFormPage> createState() => _InternetFormPageState();
}

class _InternetFormPageState extends ConsumerState<InternetFormPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(internetFormViewModelProvider.notifier).init(
  ...
  void _showConfirmationSheet(BuildContext context) {
    final viewModel = ref.read(internetFormViewModelProvider.notifier);
    final state = ref.read(internetFormViewModelProvider);
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) {
        return PPOBConfirmationSheet(
          ...
          provider: state.providerName.toUpperCase(),
          productLabel: 'Nama Pelanggan Internet',
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(internetFormViewModelProvider);
    final viewModel = ref.read(internetFormViewModelProvider.notifier);
    const primaryColor = Color(0xff8B5CF6);

    ref.listen<String?>(
      internetFormViewModelProvider.select((s) => s.errorMessage),
      (prev, next) {
                ...
                children: [
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'No. Pelanggan / ID ${state.providerName}',
                                style: const TextStyle(
                                  fontSize: 13,
                                  fontWeight: FontWeight.bold,
                                  color: Color(0xff0F172A),
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: 'Masukkan Nomor Pelanggan / ID Internet',
                            hintStyle: TextStyle(
                  ...
                  if (inquiry != null) ...[
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildDetailRow('Layanan', state.providerName),
                          const Divider(height: 20, color: Color(0xffF1F5F9)),
                          _buildDetailRow('Nama Pelanggan', customerName),
                          const Divider(height: 20, color: Color(0xffF1F5F9)),
                          _buildDetailRow('No. Pelanggan', state.customerNumber),
                          const Divider(height: 20, color: Color(0xffF1F5F9)),
