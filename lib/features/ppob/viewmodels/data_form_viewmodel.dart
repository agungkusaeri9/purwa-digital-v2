...
import 'data_form_state.dart';
import 'pulsa_form_viewmodel.dart'; // Reuse ppobServiceProvider

final dataFormViewModelProvider =
    StateNotifierProvider.autoDispose<DataFormViewModel, DataFormState>((ref) {
  final service = ref.watch(ppobServiceProvider);
  return DataFormViewModel(service);
});

class DataFormViewModel extends StateNotifier<DataFormState> {
  final PPOBService _service;

  DataFormViewModel(this._service) : super(DataFormState());
  ...
  Future<void> fetchProducts(String provider) async {
    ...
    try {
      final list = await _service.getProducts(category: 'Data', brand: provider);
